...
class Comments(db.Model):
    ...
    def by_post(cls, post):
        query = 'select * from Comments where post = :1 order by created desc'
        key = db.GqlQuery(query, post)
        return key
...
class Likes(db.Model):
    ...
    def liked(cls, post, user):
        query = 'select * from Likes where post = :1 and user = :2'
        key = db.GqlQuery(query, post, user)
        return key
...
class BlogFront(BlogHandler):
    def get(self):
        ...
        posts = greetings = db.GqlQuery(query)
            ...
            if self.request.get('delete'):
                if post.author_id == str(self.user.key().id()):
                    ...
                    self.render("permalink.html",
                                ...
                                likes=likes, liked=liked, error=error)
            ...
            if self.request.get('comment'):
                ...
                if comment_text:
                    ...
                    self.render("permalink.html", post=post,
                                ...
                                liked=liked, comment_error=comment_error)
            ...
            if self.request.get('like'):
                if post.author_id != str(self.user.key().id()):
                    if not Likes.liked(post, self.user).count():
                        ...
                        self.render("permalink.html", post=post,
                                    comments=comments, likes=likes,
                                    liked=liked, error=error)
                else:
                    error = "Cannot like own posts"
                    self.render("permalink.html", post=post,
                                comments=comments, likes=likes,
                                liked=liked, error=error)
...
class EditPost(BlogHandler):
    def get(self, post_id):
        ...
        self.render("newpost.html", heading="Edit Post",
                    subject=subject, content=content)
